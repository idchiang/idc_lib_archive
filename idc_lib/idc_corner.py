...
from matplotlib.colors import LinearSegmentedColormap, colorConverter, LogNorm
...
__all__ = ["corner_m", "hist2d", "quantile"]


def corner_m(xs, bins=20, range=None, weights=None, color="k",
             smooth=None, smooth1d=None,
             labels=None, label_kwargs={"fontsize": 16},
             logscale=False, show_titles=True, title_fmt=".2f",
             title_kwargs={"fontsize": 16},
             truths=None, truth_color="#4682b4",
             scale_hist=False, quantiles=None, verbose=False, fig=None,
             max_n_ticks=5, top_ticks=False, use_math_text=False,
             reverse=False, hist_kwargs=None, **hist2d_kwargs,
             ):
    """
    ...
    **hist2d_kwargs
    ...
    for i, x in enumerate(xs):
        ...
        # Plot the histograms.
        if smooth1d is None:
            n, _, _ = ax.hist(x, bins=bins[i], weights=weights, log=logscale,
                              range=np.sort(range[i]), **hist_kwargs)
        else:
            if gaussian_filter is None:
                raise ImportError("Please install scipy for smoothing")
            n, b = np.histogram(x, bins=bins[i], weights=weights, log=logscale,
        ...
        if i == 0:
            ax.set_ylabel(r'$\log_{10}$Likelihood', **label_kwargs)
        ...
        for j, y in enumerate(xs):
            ...
            hist2d(y, x, ax=ax, range=[range[j], range[i]], weights=weights,
                   ...
                   logscale=logscale, **hist2d_kwargs)
            """
            H, X, Y = np.histogram2d(x.flatten(), y.flatten(), weights=weights)
            ax.hist2d(y, x, range=[range[j], range[i]], weights=weights,
                      bins=[bins[j], bins[i]], cmap='Greys', cmin=H.max()/1E6,
                      norm=LogNorm())
...
def hist2d(x, y, bins=20, range=None, weights=None, levels=None, smooth=None,
    ...
    locator = LogLocator() if logscale else None
    ...
    if (plot_contours or plot_density) and not no_fill_contours:
        ax.contourf(X2, Y2, H2.T, [V.min(), H.max()], locator=locator,
                    cmap=white_cmap, antialiased=False)
        X3, Y3 = np.meshgrid(Y2, X2)
        print(X2.shape, Y2.shape, H2.shape, X3.shape, Y3.shape)
        ax.matshow(H2, norm=norm, cmap=white_cmap)
    ...
    if plot_contours and fill_contours:
        ...
        ax.contourf(X2, Y2, H2.T, np.concatenate([[0], V, [H.max()*(1+1e-4)]]),
                    locator=locator, **contourf_kwargs)
    ...
    if plot_contours:
        ...
        ax.contour(X2, Y2, H2.T, V, locator=locator, **contour_kwargs)
