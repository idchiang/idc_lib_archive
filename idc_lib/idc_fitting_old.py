...
def SE_calibration_vs_G14(nop=6):
    ...
    #
    print('################################################')
    print('            Calculating: my grid                ')
    print('################################################')
    for cov_method in range(2):
        s = pd.Series()
        logsigma_step = 0.025
        min_logsigma = -4.
        max_logsigma = 1.
        ...
        for iter_ in range(2):
            ...
            print('Best fit kappa160:', kappa160)
            wl_complete = np.linspace(1, 1000, 1000)
            # bands = ['PACS_100', 'PACS_160', 'SPIRE_250', 'SPIRE_350',
            #          'SPIRE_500']
            #
            ...
            model_complete = SEMBB(wl_complete, MWSigmaD, T, beta,
                                   kappa160=kappa160)
            # gordon_integrated = \
            #     z0mg_RSRF(wl_complete, SEMBB(wl_complete, MWSigmaD, 17.2,
            #                                  1.96, 9.6 * np.pi), bands)
            model_gordon = SEMBB(wl_complete, MWSigmaD, 17.2,
                                 1.96, 9.6 * np.pi)
        #
        s.name = 'MyGrid_'
        ...
        s['beta_EXP'] = beta
        s['kappa_EXP2'] = 10**logsigma / MWSigmaD
        df = df.append(s)
        fig = corner(samples.T, labels=labels, quantities=(0.16, 0.84),
                     show_titles=True, title_kwargs={"fontsize": 12},
                     weights=pr.flatten())
        fn = 'output/_CALI_Corner_SE_MyGrid_'
        ...
        fn = 'output/_CALI_Model_SE_MyGrid_'
        fn += 'G14COV_' if cov_method else 'MyCOV_'
        fn += '.pdf'
        with PdfPages(fn) as pp:
            pp.savefig(fig)
