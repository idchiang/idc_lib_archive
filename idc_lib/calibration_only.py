...
def exp_LSE(var, chi2):
    logZ = LSE(chi2)
    LS = LSE(chi2 + np.log10(var))
    expected = 10**(LS - logZ)
    return expected
...
def SE_calibration_vs_G14(nop=6):
    ...
    for kappa_0 in [1, 11.6 * np.pi]:
        ...
        Ts_1d = 10**np.arange(min_logT, max_logT, logT_step)
        ...
        def fitting_model(wl):
            return SEMBB(wl, sigmas, Ts, betas, kappa160=kappa_0)
        ...
        logsigmas += np.log10(kappa_0)
        logsigmas_1d += np.log10(kappa_0)
        ...
        #
        kappa160LSE = exp_LSE(kappa160s, -0.5 * chi2)
        TLSE = exp_LSE(Ts, -0.5 * chi2)
        betaLSE = exp_LSE(betas, -0.5 * chi2)
        samples = np.array([logkappa160s.flatten(), Ts.flatten(),
                            betas.flatten()])
        labels = [r'$\log\kappa_{160}$', r'$T$', r'$\beta$']
        T, T_err = exp_and_error(Ts, pr, 'T')
        logT, logT_err = exp_and_error(np.log10(Ts), pr, 'logT')
        ...
        s['T_EXPQ'] = TQ
        s['beta_EXPQ'] = betaQ
        s['kappa_EXP_LSE'] = kappa160LSE
        s['T_EXO_LSE'] = TLSE
        s['beta_EXP_LSE'] = betaLSE
        #
        df = df.append(s)
        """
