...
# 03/08/17 E. MOmjian modified for CASA 5.0.0 pre-release:
#          Currently under testing/verification
######################################################################
...
pipepath='/home/idchiang/Data_reduction/pipeline/'
...
execfile(pipepath + 'idchiang_startup.py')
...
try:
    ...
    # Only keep some spw
    if fewer_spw_ans.lower() != 'n':
        new_ms = SDM_name + 'spw_' + few_spws + '.ms'
        split(vis=ms_active, outputvis=new_ms, spw=few_spws, datacolumn='data')
        msname = new_ms
        ms_active = new_ms
        execfile(pipepath + 'EVLA_pipe_msinfo.py')
    ...
    if rm_Galatic_HI:
        flagdata(vis=msname, spw=default_rm_spw, field=default_rm_field)
        logprint ("Remove spw=" + default_rm_spw + ", field=" + default_rm_field, logfileout='logs/import.log')
    ...
    # FLAG POSSIBLE RFI ON CUSTOMIZED SCRIPT
    execfile(pipepath + 'idchiang_flagdata.py')
    flag_HI(spws='0', c_field='0~1', t_field='2~8')
    ...
    # Separate spw and subtract continuum
    # execfile(pipepath + 'idchiang_post_cali.py')
    # post_cali(arget='IC342', spw='8', exclude_chan='1600~2700')
