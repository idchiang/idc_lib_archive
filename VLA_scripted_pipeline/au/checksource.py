...
def version(short=False):
    ...
    """
    myversion = "$Id: checksource.py,v 1.19 2018/01/26 19:01:18 thunter Exp $"
    if (short):
...
def checksource(overwrite=True, verbose=False, subdir='', splitcal_vis=''):
    """
    Images the phasecal and check source in a dataset and reports statistics.
    Expects to find the *.split.cal measurement set and the corresponding
    .fluxscale file for it.
    Inputs:
...
#        imsize = int(au.nextValidImsize(int(5.0/floatcell))) # valid when we only had checksources for synthBeam < 0.25
        ...
        print "imsize = ", imsize
        region='circle[[%dpix , %dpix], 15pix ]' % (imsize/2,imsize/2)
        ...
        selfflux=au.fluxscaleParseLog(selffluxfile,field=phase)[0][0]
