...
if startdate <= 55978.50:
    ...
    if (len(bandpass_state_IDs) == 0):
        ...
        if (len(bandpass_field_list) > 1):
            if SDM_name in ['14A-468.sb29806206.eb30086747.57019.45709732639', '14A-468.sb29806206.eb30092752.57025.430288171294']:
                logprint ("WARNING: NGC2787 bandpass selection special case.", logfileout='logs/msinfo.log')
                bandpass_field_select_string = str(bandpass_field_list[1])
            else:
    ...
    if (len(delay_state_IDs) == 0):
        ...
        if (len(delay_field_list) > 1):
            if SDM_name in ['14A-468.sb29806206.eb30086747.57019.45709732639', '14A-468.sb29806206.eb30092752.57025.430288171294']:
                logprint ("WARNING: NGC2787 bandpass selection special case.", logfileout='logs/msinfo.log')
                delay_field_select_string = str(delay_field_list[1])
            else:
...
if temp[-2] != 'reduced':
    ...
    vis=ms_active
    inpmode='list'
    inpfile=ms_active[:-3] + '.flagcmds.txt'
    action='plot'
