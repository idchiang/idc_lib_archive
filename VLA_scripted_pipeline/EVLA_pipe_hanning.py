...
#    if (os.path.exists(mshsmooth) == False):
        ...
        spw='0~7'
#        outputvis=mshsmooth
        outputvis='temphanning0.ms'
        hanningsmooth()
        myHanning="n"

        # idc modification
        logprint ("Split spetral line spws from the original VIS")
        split(vis=msname, spw='8~12', datacolumn='data', outputvis='tempsplit.ms')

        logprint ("Concat spetral line spws to the smoothed continuum")
        concat(vis=['temphanning0.ms', 'tempsplit.ms'], concatvis='temphanning.ms', timesort=True)

        logprint ("Removing temp ms files")
        shutil.rmtree('tempsplit.ms')
        shutil.rmtree('temphanning0.ms')
        # idc modification
